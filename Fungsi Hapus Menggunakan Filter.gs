...
function modulFungsiHapusDataDenganKriteria() {
  ...
  const isLolosFilter = (row) => {
    ...
    if (filterOptions.kategori.aktif) {
      if (idx.kategori !== -1) {
        ...
        const allowed = filterOptions.kategori.nilaiDiperbolehkan.map((v) =>
          v.toLowerCase(),
        );
    ...
    if (filterOptions.tahun.aktif && idx.tahun !== -1) {
      ...
      if (
        isNaN(val) ||
        val < filterOptions.tahun.min ||
        val > filterOptions.tahun.max
      )
        return false;
    }
    ...
    if (filterOptions.halaman.aktif && idx.halaman !== -1) {
      ...
      if (
        isNaN(val) ||
        val < filterOptions.halaman.min ||
        val > filterOptions.halaman.max
      )
        return false;
    }
    ...
    if (filterOptions.harga.aktif && idx.harga !== -1) {
      ...
      if (
        isNaN(val) ||
        val < filterOptions.harga.min ||
        val > filterOptions.harga.max
      )
        return false;
    }
  ...
  if (dataLolos.length === 0) {
    ...
    if (allSheets.length > 1) {
      Logger.log(
        "🗑 Semua data tidak memenuhi syarat. Menghapus sheet: " + name,
      );
  ...
  if (lastDataRow < sheet.getMaxRows()) {
    ...
    Logger.log(
      `🧹 Menghapus format & isi dari ${sheet.getMaxRows() - lastDataRow} baris di bawah data.`,
    );
  }
  ...
  Logger.log(
    `✅ Selesai. Dihapus ${data.length - dataLolos.length} baris, tersisa ${dataLolos.length}.`,
  );
}

function modulHapusBarisDariHasilSeleksi(namaPenerbit) {
  const sheet =
    SpreadsheetApp.getActiveSpreadsheet().getSheetByName("Hasil Seleksi");
