...
function FungsiIsiSheetReferensi() {
  ...
  const barisTarget = [];
  const dataKode = sheetRef
    .getRange(rowAwalKode, 1, sheetRef.getLastRow() - rowAwalKode + 1, 1)
    .getValues();
  ...
  for (let i = 0; i < dataKode.length; i++) {
    ...
    if (
      isi &&
      typeof isi === "string" &&
      !isi.toLowerCase().includes("total")
    ) {
  ...
  barisTarget.forEach((row) => {
    sheetRef
      .getRange(row, 3)
      .setFormula(`=SUM(E${row}:${lastColLetter}${row})`);
  });
  //
  ...
  for (let col = startCol; col <= lastHyperlinkCol; col++) {
    ...
    if (!sheetTarget) {
      Logger.log(
        `❌ Kolom ${col} - Sheet target dengan gid ${gid} tidak ditemukan`,
      );
      continue;
    }
