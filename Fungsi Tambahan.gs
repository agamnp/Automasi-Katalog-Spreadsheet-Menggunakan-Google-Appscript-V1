...
function CariUUID() {
  ...
  SpreadsheetApp.getActiveSpreadsheet().toast(
    `✅ UUID berhasil diisi untuk ${filledCount} baris dari semua sheet!`,
    "Selesai",
  );
}
//     ========     Fungsi Cari UUID dari Katalog General     ========
  ...
  sheets.forEach((sheet) => {
    if (!skipSheets.includes(sheet.getName())) {
      ...
      if (targetCol > 0 && lastRow > headerRow) {
        ...
        sheet
          .getRange(headerRow + 1, targetCol, lastRow - headerRow, 1)
          .clearContent();
        Logger.log(
          `Data di sheet ${sheet.getName()} pada kolom HARGA SATUAN telah dihapus.`,
        );
      }
    }
  });
...
function CekKetersediaanFastbyUUID() {
  ...
  for (let sh of ss.getSheets()) {
    ...
    const uuidList = sh
      .getRange(dataStartRow, uuidColumnIndex, lastRow - dataStartRow + 1, 1)
      .getValues();
  ...
  for (let i = 1; i < dataCek.length; i++) {
    ...
    if (uuidIndex[uuid.toString().trim()]) {
      sheetCek
        .getRange(i + 1, ketersediaanCol)
        .setValue("Tersedia Di Katalog General");
      found++;
    }
  }
...
function Gabungsemuasheet() {
  ...
  const sheets = ss
    .getSheets()
    .filter((s) => !sheetExcluded.includes(s.getName()));
  ...
  const targetSheet =
    ss.getSheetByName(targetName) || ss.insertSheet(targetName);
...
function ListNamaSheet() {
  ...
  // Isi data
  const data = sheets.map((sheet, index) => [index + 1, sheet.getName()]);
