...
function ModulHapusSudahdibeli(type) {
  ...
  const headers = sheetData
    .getRange(headerRow, 1, 1, sheetData.getLastColumn())
    .getValues()[0]
    .map((h) => String(h).toLowerCase().trim());
  const data = sheetData
    .getRange(startRow, 1, numRows, sheetData.getLastColumn())
    .getValues();
  ...
  if (type === "ISBN") {
    ...
    const isbnElektronikIndex = headers.findIndex((h) =>
      h.includes("isbn elektronik"),
    );
    ...
    filteredData = data.filter((row) => {
      const valCetak =
        isbnCetakIndex !== -1
          ? String(row[isbnCetakIndex]).replace(/\D/g, "")
          : "";
      const valElektronik =
        isbnElektronikIndex !== -1
          ? String(row[isbnElektronikIndex]).replace(/\D/g, "")
          : "";
  ...
  if (filteredData.length > 0) {
    sheetData
      .getRange(startRow, 1, filteredData.length, sheetData.getLastColumn())
      .setValues(filteredData);
  }

  const deletedCount = data.length - filteredData.length;

  // ✅ Hapus sheet kalau kosong
  if (filteredData.length === 0) {
    const namaPenerbit = sheetData
      .getName()
      .replace(/^\d+\.\s*/, "")
      .trim();
  ...
  Logger.log(
    `✅ [${type}] Selesai. Total: ${data.length}, dihapus: ${deletedCount}, tersisa: ${filteredData.length}`,
  );
...
function ModulHapusSudahdibeliALLSHEET(mode, sheetMulai) {
  ...
  semuaSheet.slice(mulai).forEach((sheet) => {
    ...
    if (sheetDikecualikan.includes(namaSheet)) {
      Logger.log("➡️ Skip sheet: " + namaSheet);
      return;
    }
    spreadsheet.setActiveSheet(sheet);
    try {
      if (mode === "ISBN") {
        HapusPembeliansebelumdenganISBN(); // ✅ jalankan berdasarkan ISBN
      } else if (mode === "UUID") {
        HapusPembeliansebelumdenganUUID(); // ✅ jalankan berdasarkan UUID
      } else {
        Logger.log("⚠️ Mode tidak dikenali: " + mode);
      }
    } catch (err) {
      Logger.log("⚠️ Error di sheet " + namaSheet + ": " + err.message);
    }
  });
  Logger.log("✅ Selesai Hapus Data Sudah Di beli Menggunakan : " + mode);
}
